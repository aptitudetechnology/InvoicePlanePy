document.addEventListener('DOMContentLoaded', () => {
    ...
    newRow.innerHTML = `
      ...
      <td>
        <select class="form-select form-select-sm tax-rate-select" name="items[${index}][tax_rate]" data-calculate="item-total">
          ...
          ${window.taxRates ? window.taxRates.map(taxRate => `<option value="${taxRate.rate}">${taxRate.name} (${taxRate.rate}%)</option>`).join('') : ''}
        </select>
      </td>
  ...
  document.addEventListener('productsSelected', function(event) {
    console.log('productsSelected event received in quotes.js:', event.detail);
